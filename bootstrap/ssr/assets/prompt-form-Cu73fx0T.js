...
import { C as Card, a as CardHeader, b as CardTitle, c as CardContent } from "./card-CqYzDAC4.js";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        zt.success("Prompt generated successfully!");
      } else {
        zt.error(result.message || "Failed to generate prompt");
      }
    } catch (error) {
      console.error("Error generating prompt:", error);
      zt.error("Failed to generate prompt");
    } finally {
  ...
  const handleSubmit = (e) => {
    ...
    post(promptsStore().url, {
      onSuccess: () => {
        reset();
        setGeneratedPrompt("");
        zt.success("Prompt saved successfully!");
      },
      onError: (errors2) => {
        console.error("Validation Errors:", errors2);
        zt.error("Failed to save. Check requirements.");
      }
    });
  };
