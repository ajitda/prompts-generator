...
const IdeaGenerator = ({
  ...
  const handleGenerateIdeas = async () => {
    ...
    try {
      ...
      const formattedIdeas = Array.isArray(data.ideas) && typeof data.ideas[0] === "string" ? data.ideas.map((title) => ({
        Title: title,
        Difficulty: "Medium"
      })) : data.ideas;
...
const VideoScriptGenerator = ({
  ...
  const [plainScript, setPlainScript] = reactExports.useState("");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mx-auto mt-4 w-full max-w-4xl", children: [
    ...
    step === "script" && !isLoading && (scenes.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      SceneView,
      {
        selectedIdea: selectedConcept?.Title || "",
        scenes,
